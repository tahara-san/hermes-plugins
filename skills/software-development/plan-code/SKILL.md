...
5. Apply `planning-workflows/references/review-finding-disposition-and-convergence.md` to every phase, batch, and holistic review gate. Remediate only findings that satisfy the blocker predicate; record exactly one disposition for every other reviewer item in a round ledger saved as review-evidence under `tasks/<task-name>/reviews/`. Advisory-only reviewer output completes the gate with **no** source/test/task-doc mutation, no regenerated bundle, and no lane rerun. Judged-product changes stale approvals; review-evidence writes do not and are validated deterministically instead of by another substantive review. Never override a required lane's non-PASS in the aggregate: adjudicate, run one bounded same-byte clarification rerun, then escalate. Process failures stay fail-closed and are never parked. Default to three bundle-mutating remediation rounds per gate, then enter convergence mode and escalate after one bounded additional blocker round; a late critical finding still blocks. Require per-phase dual-lane gates for high-risk boundaries and allow low-risk phases to share one milestone review, with the holistic final review always mandatory.
