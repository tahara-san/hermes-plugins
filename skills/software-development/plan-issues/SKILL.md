...
7. **Two independent lanes within one task.** For the current immutable digest, launch every required independent review lane before waiting: Codex interactive TUI GPT-6 Astra/xhigh in managed `tmux`, plus interactive Claude Code Fable 5.1/xhigh through `claude-i`. Preserve each complete task-local transcript, but accept it only when it contains exactly one canonical `BEGIN_REVIEW_RESULT`/`END_REVIEW_RESULT` block whose digest, mode, model, effort, and verdict exactly match the recorded lane. The helper rehashes the transcript and bundle before aggregation. Never substitute `delegate_task`, `codex exec`, or `codex review` for the Codex lane.
8. **Bounded current-only rerounds and meta-reconciliation.** Save both complete ordinary results before editing. If exactly one lane passes, launch both independent same-digest cross-assessments before waiting: the **passing lane** meta-reviews the **failing lane** verdict, and the **failing lane** meta-reviews the **passing lane** verdict, each using only `UPHOLD` (agree with the verdict assessed) or `OBJECT` (dispute it). Approve only when both agree in the passing direction: passing-lane `OBJECT` plus failing-lane `UPHOLD`. Every other complete pair preserves the failure. Preserve compact unresolved opinions/findings in the **next dual-lane round** bundle context. Meta-reviews do not count toward the **six dual-lane review rounds**. Consolidate blocker fixes into one current-only amendment whose bundle contains latest authoritative state, a concise finding-to-fix delta, and compact reconciliation context. Prior raw review artifacts and meta-review artifacts remain historical by path/hash and are not embedded. There is no separate artifact-consistency review; chain integrity is validated mechanically. If round six does not pass, stop before round seven and **ask the user to decide** how to proceed.
9. **Hash-bound finality.** Archive delayed old-digest results as superseded. They cannot update current status or trigger reruns by themselves. Write `final-review.json` only when both current-digest ordinary lanes pass or a complete same-digest passing-lane `OBJECT` plus failing-lane `UPHOLD` pair validly normalizes the failing lane to PASS, and the live authoritative docs still match. Refresh status before trusting a saved ledger so post-approval bundle/doc changes are marked stale mechanically.
...
Its CLI validates graph initialization, generates one manifest-bounded bundle, records strictly attested hash-bound ordinary and meta-review verdicts, validates the complete bundle→transcripts→reconciliation→aggregate→final approval chain, freezes dependency contracts, records blocked/waived closure, updates corrected ordering metadata, and explicitly adopts mapped legacy conversions. The canonical reference contains command examples and compatibility rules.
