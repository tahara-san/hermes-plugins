...
7. **Supersede and rerun only after a predicate-satisfying suggestion is adopted.** Classify each non-blocking suggestion with the blocker predicate in `review-finding-disposition-and-convergence.md`. If one qualifies, save passing-but-superseded raw reviews and bundles, patch only task docs, regenerate the bundle, and rerun both required plan-doc review legs. If none qualifies — cosmetic, readability, optional hardening, speculation — record one disposition per finding in the aggregate and stop; the approval stands and no rerun is warranted.
