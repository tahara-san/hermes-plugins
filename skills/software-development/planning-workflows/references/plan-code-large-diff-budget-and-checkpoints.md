...
6. **Budget ordinary review rounds, not just tool calls.** Limit each gate to six dual-lane review rounds; same-digest meta-reviews and process retries without a substantive pair do not count. After an unresolved round, stop optional improvements, freeze delta scope to blocker closure and semantically affected neighbors, and carry compact reconciliation context forward. If round six does not approve the gate, stop before round seven and ask the user to decide.
...
- Six ordinary dual-lane rounds have completed without approval and new material findings keep arriving. That is a user-decision checkpoint, not a reason for round seven.
