...
7. **Rerun every mandatory review lane.**
   ...
   - Launch the fresh interactive Codex TUI and Claude Code Fable 5.1 @ xhigh effort lanes before waiting on either. If one lane is blocked, the other may finish independently, but the phase remains incomplete until all mandatory lanes pass or the user explicitly waives/replaces one.

## Blocked reviewer artifacts

If an interactive Claude Code Fable 5.1 @ xhigh effort lane is blocked before reading the bundle (for example `Not logged in · Please run /login`):
