...
4. Within the current task, launch every required independent review lane before waiting: Codex interactive TUI GPT-6 Astra/xhigh in managed `tmux`, and interactive Claude Code Fable 5.1/xhigh through `claude-i`, both against the same digest.
5. Save both complete hash-bound ordinary results before editing. If exactly one lane passes, launch both independent same-digest cross-assessments before waiting: the **passing lane** reviews the **failing lane** verdict, and the **failing lane** reviews the **passing lane** verdict, each using only `UPHOLD` (agree with the verdict assessed) or `OBJECT` (dispute it). Approve only for passing-lane `OBJECT` plus failing-lane `UPHOLD`; every other complete pair preserves the failure. An unresolved result proceeds to the **next dual-lane round** with compact reconciliation opinions/findings. Consolidate blocker remediation in a current-only bundle containing latest authoritative evidence and a concise delta/context. Prior raw review artifacts and meta-review artifacts remain unbundled history. There is no separate artifact-consistency review; the helper validates the complete chain mechanically. Limit the task gate to **six dual-lane review rounds**; meta-reviews do not count. Non-blocking suggestions remain implementation-review attention points and do not force document churn.
...
7. If the sixth ordinary round does not pass, stop before round seven and ask the user to decide how to proceed. Do not start another review round without an explicit user decision.
