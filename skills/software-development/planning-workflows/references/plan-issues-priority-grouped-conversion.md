...
5. Save both complete hash-bound ordinary results before editing. If exactly one lane passes, ask the **passing lane** to **meta-review** the **failing lane** against the same digest using only `UPHOLD` (agree with the prior verdict) or `OBJECT` (dispute it). Passing-lane `UPHOLD` preserves the failure. Passing-lane `OBJECT` goes to failing-lane reconsideration; its `UPHOLD` retains the failure, while its `OBJECT` withdraws the failure and normalizes that lane to PASS. An unresolved result proceeds to the **next dual-lane round** with compact reconciliation opinions/findings. Consolidate blocker remediation in a current-only bundle containing latest authoritative evidence and a concise delta/context. Prior raw review artifacts and meta-review artifacts remain unbundled history. There is no separate artifact-consistency review; the helper validates the complete chain mechanically. Limit the task gate to **six dual-lane review rounds**; meta-reviews do not count. Non-blocking suggestions remain implementation-review attention points and do not force document churn.
