...
10. Because `plan-issues` invokes task-doc creation, run the explicit `plan-doc` review gate on the generated plans unless the user explicitly waives it. Do not treat embedded future review-gate wording in `spec.md` as satisfying this requirement. Build a self-contained plan-doc bundle for the generated task docs, run both required review legs (Codex-style delegate + Claude Code default Opus 4.8 via `claude-i`), save raw artifacts and an aggregate verdict under each generated task's `reviews/`, and rerun both legs if reviewer feedback changes docs.
11. When one `plan-issues` conversion produces multiple task directories, review them one by one unless the user explicitly asks for a combined review bundle. Do not move to the next generated task until the current task's mandatory review gate has passed or is explicitly waived/blocked. If a Codex-style delegate is dispatched but has not returned, save a pending artifact naming the delegation id and bundle path, and report the conversion as pending rather than complete.
