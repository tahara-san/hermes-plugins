...
6. **Rerun every mandatory review lane on the regenerated bundle.** Launch the interactive Codex TUI and Claude Code Fable 5.1 @ xhigh effort lanes against that bundle before waiting on either. A prior approval from before the reviewer-driven source/test fix is stale. Do not update TODO/progress to complete until both fresh lanes pass.
...
8. **For interactive Claude Code Fable 5.1 @ xhigh effort, bound read-only exploration.** Approve prepared bundle reads and narrowly relevant source/status checks. If it roams after several checks, interrupt and request a no-tools verdict from reviewed context.
