...
7. Launch the pinned interactive Codex TUI against the immutable bundle using bare `codex` in managed tmux, GPT-6 Astra @ xhigh, and the read-only/artifact contract in `codex-cli-review-lane.md`. Save the CLI version, session name, model/effort banner, raw pane capture, bundle path/hash, and normalized fail-closed verdict. A started session or partial pane is not approval.
