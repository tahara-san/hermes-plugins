...
3. **Avoid staling the just-passed review.** After a final artifact-consistency review passes, do not patch reviewed task docs, historical bundles, or superseded pending markers just to make scans prettier. A judged-product edit after the review stales the verdict. Writing the canonical verdict, disposition ledger, or other review-evidence file does not: validate it deterministically (schema, referenced files, digest/size, model/effort and bundle identity, secret scan, no pending placeholders) rather than reviewing it again. See `review-finding-disposition-and-convergence.md`. Prefer writing a self-excluded/canonical verdict artifact or updating ephemeral session TODO state.
