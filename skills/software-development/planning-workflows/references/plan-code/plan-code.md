...
4. Save separate Codex interactive TUI GPT-5.6 SOL @ xhigh review and Claude Code Fable 5 @ xhigh effort (`claude-i`) artifacts plus an aggregate verdict for the phase/batch review when the plan requires durable review artifacts. The aggregate verdict must record bundle path, reviewer tool/model, both verdicts, static-scan status, verification status, and timestamp.
...
When exactly one required lane passes and the other returns `CHANGES_REQUIRED`, do not override either verdict or change judged bytes. Ask the **passing lane** to **meta-review** the **failing lane** against the same immutable digest. Meta verdicts are only `UPHOLD` (agree with the verdict assessed) and `OBJECT` (dispute it). Passing-lane `UPHOLD` preserves the failure and leads to remediation and the **next dual-lane round**; passing-lane `OBJECT` is sent to the failing lane for reconsideration. Failing-lane `UPHOLD` retains the failure and carries compact reconciliation context into the next dual-lane round; failing-lane `OBJECT` withdraws its failure, normalizes that lane to PASS, and approves the same bundle. A `BLOCKED_PROCESS` lane — wrong model/effort, bad digest, malformed output or preamble before the verdict, missing coverage, missing raw pane, unavailable CLI, unauthorized scope — is never meta-reviewed, parked, or downgraded; it stays fail-closed.
...
3. Run the default parallel Codex interactive TUI GPT-5.6 SOL @ xhigh review + Claude Code Fable 5 @ xhigh effort (`claude-i`) review stack on the fix.
...
4. Report:
   ...
   - verification result
   - number of simplify and review iterations, plus the final aggregate review artifact path and verdict for Codex interactive TUI GPT-5.6 SOL @ xhigh review + Claude Code Fable 5 @ xhigh effort (`claude-i`)
   - files changed
...
- Letting an aggregate artifact declare the gate passed while a required lane still has an unresolved `CHANGES_REQUIRED`. Use the same-digest passing-lane/failing-lane `UPHOLD`/`OBJECT` meta-review chain; only the failing lane's own `OBJECT` withdraws its failure — the aggregate never overrides it.
