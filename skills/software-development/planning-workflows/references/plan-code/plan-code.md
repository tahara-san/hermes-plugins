---
name: plan-code
description: Use when the user wants Hermes to execute an existing implementation plan (Claude Code /plan-code style). Loads tasks/<task-name> plan docs or an in-context plan, evaluates serial vs parallel execution, implements safe independent batches simultaneously, enforces decision gates, runs simplify before the default parallel Codex interactive TUI GPT-6 Astra @ xhigh review + Claude Code Fable 5.1 @ xhigh effort (`claude-i`) review gate, verifies builds/tests, updates progress, and reports completion. Includes guidance for E2E fixture prerequisite gating in references/e2e-fixture-prerequisite-gating.md.
version: 1.2.0
...
4. Save separate Codex interactive TUI GPT-6 Astra @ xhigh review and Claude Code Fable 5.1 @ xhigh effort (`claude-i`) artifacts plus an aggregate verdict for the phase/batch review when the plan requires durable review artifacts. The aggregate verdict must record bundle path, reviewer tool/model, both verdicts, static-scan status, verification status, and timestamp.
...
Track ordinary review rounds per gate. Limit the gate to **six dual-lane review rounds**; meta-reviews do not count, and process retries that never produce a substantive pair do not count. After an unresolved round, stop optional improvements, disable broad unrelated simplify edits, deduplicate by stable finding ID, and restrict review to blocker closure plus semantically affected scope. If round six does not approve the gate, stop before round seven and **ask the user to decide** how to proceed. A late critical security, data-integrity, or correctness finding is still blocking.

When exactly one required lane passes and the other returns `CHANGES_REQUIRED`, do not override either verdict or change judged bytes. Launch both independent same-digest cross-assessments before waiting: the **passing lane** reviews the **failing lane** verdict, and the **failing lane** reviews the **passing lane** verdict. Meta verdicts are only `UPHOLD` (agree with the verdict assessed) and `OBJECT` (dispute it), and either may arrive first. Approve the unchanged bundle only for passing-lane `OBJECT` plus failing-lane `UPHOLD`. Any other complete pair preserves the failure and carries compact reconciliation context into the **next dual-lane round**. A `BLOCKED_PROCESS` lane — wrong model/effort, bad digest, malformed output or preamble before the verdict, missing coverage, missing raw pane, unavailable CLI, unauthorized scope — is never meta-reviewed, parked, or downgraded; it stays fail-closed.

When the user explicitly requires Claude Code review for every `/plan-code` review gate, run it after each phase or parallel batch in addition to `simplify` and Codex interactive TUI GPT-6 Astra @ xhigh review, and run a holistic Claude Code review before final verification/cleanup. Do not treat an earlier phase Claude Code review as satisfying the holistic gate, and do not move to the next phase after patching Claude findings until affected verification plus the required review set have been rerun.
...
2. Run the default parallel Codex interactive TUI GPT-6 Astra @ xhigh review + Claude Code Fable 5.1 @ xhigh effort (`claude-i`) review stack on all changed files together, using one saved final bundle when safe.
3. Fix -> simplify -> review, but only for confirmed blockers. Advisory-only output from both lanes closes the holistic gate with no source/test/doc mutation and no rerun. Limit the gate to six dual-lane review rounds; meta-reviews and process retries without a substantive pair do not count. After an unresolved ordinary round, stop optional improvements and restrict remediation to confirmed blockers plus semantically affected scope. If round six does not approve the gate, stop before round seven and ask the user to decide.
...
3. Run the default parallel Codex interactive TUI GPT-6 Astra @ xhigh review + Claude Code Fable 5.1 @ xhigh effort (`claude-i`) review stack on the fix.
...
4. Report:
   ...
   - verification result
   - number of simplify and review iterations, plus the final aggregate review artifact path and verdict for Codex interactive TUI GPT-6 Astra @ xhigh review + Claude Code Fable 5.1 @ xhigh effort (`claude-i`)
   - files changed
...
- Letting an aggregate artifact declare the gate passed while a required lane still has an unresolved `CHANGES_REQUIRED`. Use both same-digest cross-assessments; only passing-lane `OBJECT` plus failing-lane `UPHOLD` shows both reviewers agree in the passing direction — the aggregate never overrides either lane.
- Running review before simplify, or running only one leg of the default Codex interactive TUI GPT-6 Astra @ xhigh review + Claude Code Fable 5.1 @ xhigh effort (`claude-i`) stack when both are required.
