...
3. **Adjudicate concrete findings against the live tree**
   ...
   - Then apply the blocker predicate from `review-finding-disposition-and-convergence.md`. A finding that is still true but fails the predicate — style, optional hardening, speculation, evidence polish, an unrelated pre-existing defect — gets a recorded disposition, not a fix. Only a valid finding that satisfies the predicate justifies the mutate-regenerate-rerun cycle below.
4. **If a finding is valid and blocking**
...
- Saving a late superseded review artifact (approved, failed, or pending disposition) writes to the **review-evidence plane**. It does not stale a judged-product approval and must not trigger another substantive dual-lane review. Validate it deterministically instead: schema, referenced-file existence, digest/size, model/effort and bundle-identity consistency, secret scan, no pending placeholders, task-scoped cleanup. Regenerate the bundle and dispatch a replacement review only when the late result forces a judged-product change — source, tests, fixtures, task docs, executable commands/contracts, or a contract-bearing final report. A late result that only added bookkeeping does not justify a new review round.
...
- Treat reviewer-found money/state-machine warnings as blocking under the materiality predicate: they are contract-relevant and material to financial/data integrity, so they satisfy it as long as they are grounded and name a concrete failure mode. Fix the smallest repository/service primitive instead of deferring to later webhook/cron phases, unless a product decision is genuinely required.
