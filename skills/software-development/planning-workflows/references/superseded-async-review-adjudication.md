...
- When a mandatory reviewer is explicitly model-overridden (for example Claude Code Opus 4.8 @ xhigh effort instead of the workflow default), every rerun must re-verify the model/banner before submitting the prompt and save the raw verdict separately from the structured JSON.
