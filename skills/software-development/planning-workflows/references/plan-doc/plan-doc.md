...
9. When exactly one required lane passes and the other returns `CHANGES_REQUIRED`, keep the docs and digest immutable and ask the **passing lane** to **meta-review** the **failing lane**. Meta verdicts are only `UPHOLD` (agree with the verdict assessed) and `OBJECT` (dispute it). Passing-lane `UPHOLD` preserves the failure and leads to blocker remediation and the **next dual-lane round**. Passing-lane `OBJECT` is sent to the failing lane for reconsideration: its `UPHOLD` retains the failure and carries compact reconciliation context into the next dual-lane round; its `OBJECT` withdraws the failure, normalizes that lane to PASS, and approves the same bundle. Meta-reviews do not count toward the six dual-lane review rounds. If round six does not approve the gate, stop before round seven and **ask the user to decide** how to proceed. Process failures remain fail-closed and never enter substantive reconciliation.
