...
9. When exactly one required lane passes and the other returns `CHANGES_REQUIRED`, keep the docs and digest immutable and launch both independent same-digest cross-assessments before waiting: the **passing lane** reviews the **failing lane** verdict, and the **failing lane** reviews the **passing lane** verdict. Meta verdicts are only `UPHOLD` (agree with the verdict assessed) and `OBJECT` (dispute it), and either may finish first. Approve only for passing-lane `OBJECT` plus failing-lane `UPHOLD`; every other complete pair preserves the failure and carries compact reconciliation context into the **next dual-lane round**. Meta-reviews do not count toward the six dual-lane review rounds. If round six does not approve the gate, stop before round seven and **ask the user to decide** how to proceed. Process failures remain fail-closed and never enter substantive reconciliation.
