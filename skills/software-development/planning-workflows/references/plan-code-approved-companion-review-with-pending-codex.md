# Plan-code: approved companion review while interactive Codex TUI is pending

Use when a `plan-code` implementation is otherwise complete, automated verification is green, and the companion reviewer (for example `claude-i`) approves, but the mandatory interactive Codex TUI has not produced a parseable attested verdict yet.
...
2. **Save the companion approval immediately** as raw and structured artifacts, including bundle path, observed reviewer model/tool/effort, verdict, and all non-blocking suggestions/testing gaps. For interactive Claude Code Fable 5.1 @ xhigh effort lanes, capture the pane after the verdict, write a normalized JSON/markdown verdict yourself if the reviewer used prose, and clear/exit the TUI session so queued text does not accidentally execute later.
...
4. **Create a pending Codex artifact** such as `tasks/<slug>/reviews/codex-implementation-review-pending.json` with:
   ...
   - exact resume steps.
5. **Leave aggregate review absent.** Do not create `final-review.json` until the interactive Codex TUI verdict is saved or explicitly waived.
...
8. **Final response wording:** say implementation is complete and verified, but the task is blocked/pending on the mandatory interactive Codex TUI review. Include whether the agent is stuck or not, the pending artifact path, and the exact next step.

## Resume

When the interactive Codex verdict becomes available:
