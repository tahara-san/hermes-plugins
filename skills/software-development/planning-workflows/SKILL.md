...
When the user explicitly invokes `/plan-doc`, `/plan-code`, or says to use `plan-doc` / `plan-code`, treat that as a workflow contract, not a suggestion. These flows are fail-closed: if any required artifact or gate cannot be completed, stop and report the blocker instead of continuing as ordinary implementation.
...
| **Claude Code Fable 5 @ xhigh effort review** | Claude Code CLI through the interactive `claude-i` workflow, not `claude -p`; start with `claude --model fable --fallback-model opus --effort xhigh`, then verify and record the TUI model/effort banner. | `fable` selects the latest available Fable; if it is unavailable, `opus` selects the latest available Opus. The fallback is permitted, but the artifact must name the actual model. This lane intentionally uses the local `claude` CLI because Hermes cannot directly delegate messages to Claude Code. |
...
5. Include a copy-pasteable kickoff prompt for the implementer. The kickoff prompt must mention the task directory, objective, ordered phases, the default parallel Codex interactive TUI GPT-5.6 SOL @ xhigh + Claude Code Fable 5 @ xhigh effort review stack, verification commands, out-of-scope constraints, and final-report expectations.
6. Run the default parallel review stack on the plan-doc bundle unless the user explicitly overrides review. Save the Codex raw `tmux capture-pane` artifact and schema-constrained verdict, the Claude Code Fable 5 @ xhigh effort (`claude-i`) artifact, and an aggregate plan-review verdict under `tasks/<task-name>/reviews/`. For Codex, retain the managed tmux session name and recover only through bounded pane captures; a session name, startup banner, or partial pane is not approval. Follow `references/codex-cli-review-lane.md` for exact recovery and rerun behavior.
...
5. Run the default parallel review stack before completion: Codex interactive TUI with GPT-5.6 SOL @ xhigh per `references/codex-cli-review-lane.md` plus Claude Code Fable 5 @ xhigh effort via `claude-i`, both against the same saved full implementation bundle. Launch both lanes before waiting on either one. Save the full-round clean baseline before carrying any verdict forward.
6. Save the final aggregate review verdict under `tasks/<task-name>/reviews/final-review.json` or document why an equivalent artifact path was used. The aggregate verdict must reference the Codex raw-pane and normalized-verdict artifacts and the Claude Code Fable 5 @ xhigh effort (`claude-i`) artifact, record the model/effort actually attested by each CLI, and name each review round as full or delta; the task is not complete unless both required legs pass or the user explicitly waives one.
...
- Claude Code Fable 5 @ xhigh effort (`claude-i`) review run: yes/no/not applicable
...
Use `references/plan-issues/plan-issues.md` as the canonical contract and its shipped `scripts/plan_issues_workflow.py` helper for graph initialization, metadata, the compact status ledger, fresh-session handoffs, one-slug manifest-bounded bundles with required `spec.md`/`todo.md`, strictly attested hash-bound verdicts, complete approval-chain revalidation, stale-result archival, current-only delta rerounds, the default four-total-review-round cap, compact frozen dependency contracts, metadata-only order migration, and explicit legacy adoption.

Because issue conversion invokes task-doc creation, every generated task requires the explicit `plan-doc` review gate unless the user explicitly waives it. Mapping or drafting all docs up front is allowed, but review **one current task at a time**. Do not generate or dispatch the next task until the current task is approved, explicitly waived, or durably blocked and the user authorizes moving on. Within that current task, build one immutable task-local bundle and launch every required independent review lane before waiting: interactive Codex TUI GPT-5.6 SOL/xhigh plus Claude Code Fable 5/xhigh through `claude-i`, both against the exact same digest. Require exactly one canonical result block in each preserved raw transcript. On a blocker reround, use a current-only bundle containing latest authoritative evidence plus a concise finding-to-fix delta; prior full bundles and prior raw review artifacts stay historical by path/hash and are not embedded. There is no separate artifact-consistency review: the same mechanical chain validator covers bundle, transcripts, aggregate, final artifact, and live docs. Preserve optional suggestions without churn. The default is four total review rounds; if the fourth round does not pass, stop and ask the user to decide how to proceed instead of starting another review round. A coverage map, startup pane, old `final-review.json`, or `git diff --check` is not current approval.
...
- When the user changes requirements during `/plan-code`, follow `references/mid-task-contract-changes.md`: test the new contract, remove stale compatibility paths, update plan artifacts, rerun verification, and regenerate/re-review final bundles.
